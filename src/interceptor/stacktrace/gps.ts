...
function _parseJson(string: any) {
...
function _findFunctionName(source: any, lineNumber: any, columnNumber: any) {
  var syntaxes = [
...
function _findPrev(source: any, lineNumber: any, columnNumber: any) {
  var lines = source.split("\n");

  const lineNo = lineNumber - 1;
  const prevLines = lines.slice(Math.max(lineNo - 2, 0), lineNo + 1);
  return prevLines;
}
...
function _ensureStackFrameIsLegit(stackframe: any) {
...
function _findSourceMappingURL(source: any) {
...
export class Gps {
  ...
  _get(location: any) {
    return new Promise((resolve: any, reject: any) => {
      ...
      } else if (this.opts.offline && !isDataUrl) {
   ...
   */
  _getSourceMapConsumer(sourceMappingURL: any, defaultSourceRoot: any) {
        ...
        var sourceMapConsumerPromise = new Promise(
          (resolve: any, reject: any) => {
            return this._get(sourceMappingURL)
              .then(function (sourceMapSource: any) {
   ...
   */
  pinpoint(stackframe: any) {
    return new Promise((resolve, reject) => {
      this.getMappedLocation(stackframe).then((mappedStackFrame: any) => {
   ...
   */
  findFunctionName(stackframe: any) {
    return new Promise((resolve, reject) => {
      ...
      this._get(stackframe.fileName)
        .then((source) => {
          ...
          var prevLines = _findPrev(source, lineNumber, columnNumber);
   ...
   * @returns {Promise} that resolves with enhanced StackFrame.
   */
  getMappedLocation(stackframe: any) {
    return new Promise((resolve, reject) => {
